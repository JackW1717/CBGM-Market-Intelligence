...
async function pullFredSeries(source: FredSeriesSource): Promise<NewsItem[]> {
  try {
    ...
    const publishedAt = `${latest.date}T00:00:00.000Z`;
    const link = `https://fred.stlouisfed.org/series/${source.seriesId}`;
...
async function main() {
  ...
  const merged = dedupe([...rssResults.flat(), ...fredResults.flat()]);
